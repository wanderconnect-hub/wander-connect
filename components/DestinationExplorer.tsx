...
const ExtraTravelInfo: React.FC<{ destination: string }> = ({ destination }) => {
  ...
  const handleGenerateItinerary = async () => {
    ...
    try {
      ...
      setItinerary("Failed to generate itinerary. Try again.");
    } finally {
    ...
    <div className="mt-8 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 bg-white rounded-xl shadow-md text-center">
          <h4 className="text-lg font-semibold text-cyan-700">Weather</h4>
          <p className="text-stone-600">🌤️ 22°C, Mild</p>
        </div>
        <div className="p-4 bg-white rounded-xl shadow-md text-center">
          <h4 className="text-lg font-semibold text-cyan-700">Budget</h4>
          <p className="text-stone-600">💰 $80-120/day</p>
        </div>
        <div className="p-4 bg-white rounded-xl shadow-md text-center">
          <h4 className="text-lg font-semibold text-cyan-700">Trip Duration</h4>
          <p className="text-stone-600">⏳ 3-5 days</p>
        </div>
      </div>

      {/* Nearby Destinations */}
      <div>
        <h3 className="text-xl font-bold text-stone-700 mb-3">Nearby Destinations</h3>
        <div className="flex gap-3 overflow-x-auto pb-2">
          {['Amsterdam', 'Brussels', 'Barcelona'].map((city) => (
            <div key={city} className="min-w-[150px] p-4 bg-gradient-to-r from-cyan-50 to-cyan-100 rounded-xl shadow-sm">
              <p className="font-medium text-cyan-800">{city}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Itinerary Generator */}
      <div>
        <h3 className="text-xl font-bold text-stone-700 mb-3">AI Itinerary</h3>
        <button
        ...
        >
          {loadingItinerary ? "Generating..." : "Generate 3-Day Plan"}
        </button>

        {itinerary && (
          <div className="mt-4 p-4 bg-white rounded-xl shadow-md text-stone-700 whitespace-pre-line">
            {itinerary}
          </div>
        )}
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {suggestedDestinations.map((place) => (
            <button
              ...
              className="relative aspect-square rounded-xl overflow-hidden group shadow-md hover:shadow-xl transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-cyan-500 focus:ring-opacity-75"
            ...
            >
              ...
              <div className="absolute bottom-0 p-3 w-full">
                <h3 className="text-white font-bold text-base leading-tight drop-shadow-md text-left">{place.name}</h3>
              </div>
            </button>
          ))}
        </div>
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        ...
        />
        <button type="submit" disabled={isLoading || !destination.trim()} className="bg-cyan-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-cyan-700 transition-colors disabled:bg-stone-400">
          Explore
        </button>
      </form>
