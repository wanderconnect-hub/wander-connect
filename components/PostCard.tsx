// File: components/PostCard.tsx - FINAL CORRECTED VERSION
          ...
          <div className="relative" ref={menuRef}>
            ...
            {isMenuOpen && (
              <div className="absolute top-full right-0 mt-1 bg-white rounded-lg shadow-xl border border-stone-200/80 w-40 overflow-hidden z-10 animate-fade-in-down">
                <ul>
                  <li><button onClick={handleEditClick} className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm text-stone-700 hover:bg-stone-100 transition-colors"><PencilSquareIcon className="w-4 h-4" /><span>Edit Post</span></button></li>
                  <li className="border-t border-stone-100"></li>
                  <li><button onClick={handleDeleteClick} className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors"><TrashIcon className="w-4 h-4" /><span>Delete Post</span></button></li>
                </ul>
              </div>
            )}
          </div>
      ...
      {post.mediaUrl && (
        <div className="bg-stone-100">
          {post.mediaType === 'image' ? (
            <img className="w-full h-auto max-h-[70vh] object-contain" src={post.mediaUrl} alt="Post media" />
